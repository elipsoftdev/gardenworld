...
const FINISHES: Finish[] = [
  { key: "blanco", label: "Blanco", description: "Una presencia limpia para muros luminosos.", sku: "GW-RU-BLC", color: "#f5f3ed", image: "/images/ru-blanco-45.webp" },
  { key: "piedra", label: "Piedra", description: "Un neutro cálido que acompaña materiales naturales.", sku: "GW-RU-PDR", color: "#b9aa94", image: "/images/ru-piedra-45.webp" },
  { key: "inox", label: "Inoxidable", description: "Acabado sobrio para arquitectura contemporánea.", sku: "GW-RU-INX", color: "#c7c8c4", image: "/images/ru-inox-45.webp" },
  { key: "corten", label: "Corten", description: "Un tono mineral que suma carácter al exterior.", sku: "GW-RU-COR", color: "#8c4d31", image: "/images/ru-corten-45.webp" },
...
export default function Home() {
  ...
  return <>
    ...
    <main aria-hidden={menuOpen}>
      <section className="hero" aria-labelledby="hero-title"><div className="hero-copy"><p className="eyebrow hero-eyebrow">Bases de mangueras Garden World</p><h1 id="hero-title">Tu mundo.<span>Tu jardín.</span></h1><p className="hero-intro">Diseño, orden y funcionalidad para disfrutar mejor los espacios exteriores.</p><div className="hero-actions"><a className="button" href="#bases">Conocer nuestras bases <span aria-hidden="true">↓</span></a><a className="text-link" href="#cotizar">Cotizar <span aria-hidden="true">↗</span></a></div></div><div className="hero-visual" aria-label="Base para manguera Garden World"><div className="hero-image-wrap"><img src="/images/ru-piedra-45.webp" alt="Base para manguera Garden World en acabado piedra instalada en pared exterior" /></div><div className="hero-visual-note"><span>Producto principal</span><strong>Bases</strong><small>Diseño que ordena</small></div><svg className="hero-rings" viewBox="0 0 180 180" aria-hidden="true"><circle cx="90" cy="90" r="68" /><circle cx="90" cy="90" r="48" /><circle cx="90" cy="90" r="28" /></svg></div><p className="hero-side-note">Garden World · Venezuela</p></section>
      ...
      <section className="rollup-section" id="beneficios"><div className="container rollup-grid"><div className="rollup-copy" data-reveal><p className="eyebrow eyebrow-light">Bases de mangueras Garden World</p><h2>Todo en su lugar.<br />También afuera.</h2><p className="rollup-lede">Una pieza para mantener la manguera ordenada, accesible y fuera del camino sin romper la estética de tu espacio.</p><ul className="benefit-list" aria-label="Beneficios de las bases de mangueras"><li><span>01</span>Orden visible</li><li><span>02</span>Uso simple</li><li><span>03</span>Diseño para exterior</li><li><span>04</span>Cinco acabados</li></ul><a className="button button-light" href="/productos/">Conocer nuestras bases <span aria-hidden="true">↗</span></a></div><div className="rollup-media" data-reveal><img src="/images/ru-corten-43.webp" alt="Base para manguera Garden World en acabado corten con manguera organizada" loading="lazy" decoding="async" /><div className="media-caption"><span>Bases Garden World</span><span>Acabado Corten</span></div></div></div></section>
      <section className="section finishes-section" id="acabados"><div className="container section-heading section-heading-row" data-reveal><div><p className="eyebrow">Bases Garden World · Acabados</p><h2>Una forma.<br />Tu acabado.</h2></div><p>Elige el acabado que mejor se integra con los materiales, tonos y carácter de tu espacio.</p></div><div className="container finish-layout" data-reveal><div className="finish-image">{FINISHES.map((finish) => <img key={finish.key} src={finish.image} alt={`Base para manguera Garden World en acabado ${finish.label}`} className={finish.key === finishKey ? "is-active" : ""} loading={finish.key === "piedra" ? "eager" : "lazy"} decoding="async" />)}<span className="finish-counter">0{FINISHES.findIndex((finish) => finish.key === finishKey) + 1} / 05</span></div><div className="finish-panel"><div className="finish-current"><p>Acabado seleccionado</p><h3>{selectedFinish.label}</h3><span>{selectedFinish.description}</span></div><div className="finish-options" role="group" aria-label="Elegir acabado de bases de mangueras">{FINISHES.map((finish) => <button type="button" key={finish.key} className={finish.key === finishKey ? "is-active" : ""} aria-pressed={finish.key === finishKey} onClick={() => setFinishKey(finish.key)}><span className="finish-swatch" style={{ backgroundColor: finish.color }} aria-hidden="true" /><span>{finish.label}</span><small>{finish.sku}</small></button>)}</div><a className="button button-wide" href={finishWhatsApp} target="_blank" rel="noopener noreferrer" data-whatsapp-cta><WhatsAppIcon /> Cotiza este acabado <span aria-hidden="true">↗</span></a></div></div></section>
